...
function updateResultFromResultObj(result) {
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    if (fbEl) {
      ...
      switch (i) {
        case 1:
          fbText = crit.taskResponse?.feedback || "";
          break;
        case 2:
          fbText = crit.coherence?.feedback || "";
          break;
        case 3:
          fbText = crit.lexical?.feedback || "";
          break;
        case 4:
          fbText = crit.grammar?.feedback || "";
          break;
        case 5:
          fbText = result.feedback || "";
          break;
      }
...
async function submitEssay() {
  ...
  try {
    ...
    const payload = topicId
      ? { topicId, essay: text }
      : { topic: title, essay: text };
    if (activeSubmissionId) {
      payload.submissionId = activeSubmissionId; // <-- เพิ่ม ID เข้าไปถ้ามี
    } else if (topicId) {
    ...
    const j = await res.json();
    updateResultFromResultObj(j.submission.result);

    addToHistoryItem({
      id: j.submission.id,
      topic: j.submission.topic,
      topicId: topicId || j.submission.topicId,
      essay: j.submission.essay,
      result: j.submission.result,
